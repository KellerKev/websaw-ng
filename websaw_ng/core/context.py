...
class BaseContext(metaclass=MetaContext):
    ...
    async def aget(self, k):
        """Async twin of __getattr__'s fixture resolution: awaits atake_on and
        caches the value so later (sync) ``ctx.<k>`` access returns it."""
        state = self._local.state
        in_use = state.in_use
        fixt_value = in_use.get(k)
        if fixt_value is not None:
            return fixt_value[1]
        try:
            fixt = self._fixt[k]
        except KeyError:
        ...
        self._fixture_prepare_for_use(fixt)
        try:
            value = await fixt.atake_on(self)
        except Exception as exc:
            state.exception_stack.append(exc)
            raise FixtureProcessError()
        if value is None:
    ...
    async def use_fixtures_async(self, fixt, hooks: Union[bool, dict] = False):
        for fkey in fixt:
            await self.aget(fkey)

    async def afinalize(self, exc):
        state = self._local.state
        if exc:
            state.exception_stack.append(exc)
        in_use = state.in_use
        ...
        max_cnt = 100
        while max_cnt > 0 and in_use_stack:
            max_cnt -= 1
            f = in_use_stack.pop()
            try:
                await f.atake_off(self)
            except Exception as exc:
                state.exception_stack.append(exc)
