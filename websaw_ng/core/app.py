...
class BaseApp:
    ...
    def __init__(
        ...
        # Routing is resolved through ``self._router``. By default apps share the
        # single ``globs.app`` ombott instance (one server serves every mounted
        # app, dispatched by base_url). An isolated app is given its OWN router
        # (see ``DefaultApp(isolated=True)``) — its own routes AND its own
        # request/response — so it can't collide with another app in the process.
        # ombott resolves the module-level request/response context-locally per
        # request, so ``redirect()`` / ``ctx.response`` / ``Set-Cookie`` land on
        # THIS app's response even when several isolated apps serve concurrently.
    ...
    def asgi(self):
        ...
        (equals ``globs.app.asgi`` unless the app is isolated)."""
    ...
    def websocket(self, rule: str, name: str = None, **kw):
        """Register an ASGI WebSocket controller: ``async def h(ctx, ws, **args)``.

        Runs with the app's context + fixtures (so ctx.db / ctx.session work);
        there is no render step (the controller drives the socket directly).
        Requires serving under an ASGI server (e.g. ``-s uvicorn``).
        """
        args = (rule, 'WEBSOCKET', name)
    ...
    def _mount(self, config: dict = None):
        ...
        for raw_h, meta, mixin_data in self._iter_registered():
            is_ws = any(ra[0][1] == 'WEBSOCKET' for ra in meta.routes_args)
            if is_ws:
                h = self.make_ws_handler(raw_h, meta.fixtures, context, exception_handler, mixin_data)
            else:
                maker = self.make_async_handler if iscoroutinefunction(raw_h) else self.make_handler
                h = maker(raw_h, meta.fixtures, context, render_map, exception_handler, mixin_data)
            ...
            for route_args, route_kw in meta.routes_args:
                # opt the route's body out of pre-buffering (request.stream())
                if route_kw.pop('stream', False):
                    try:
                        h.__ombott_stream__ = True
    ...
    def make_async_handler(
        ...
        """Async twin of make_handler for `async def` controllers (ASGI).

        Mirrors the sync lifecycle but awaits the async fixture hooks
        (use_fixtures_async / afinalize) and the coroutine controller/renderer.
        ombott's ASGI dispatch awaits the returned coroutine handler.
        """
        hooks = False
        if fixtures:
            hooks = {
        ...
        async def handler(**kw):
            ...
            try:
                if fixtures:
                    await ctx.use_fixtures_async(fixtures, hooks)
                out = h(ctx, **kw)
                if iscoroutine(out):
                    out = await out
                ctx.output = out
            ...
            if render_map:
                ...
                if render:
                    ...
                    ctx.output = await out if iscoroutine(out) else out
            return ctx.output
        return handler
    ...
    def make_ws_handler(
            h: Callable, fixtures: List[str], ctx: BaseContext,
            exception_handler=None, mixin_data=None
    ) -> Callable:
        """Build a WebSocket route handler: runs the context + (async) fixtures
        around an ``async def controller(ctx, ws, **kw)``; no render step. ombott
        calls the returned coroutine as ``handler(ws, **url_args)``."""
        hooks = False
        if fixtures:
            hooks = {
                fkey: fobj for fkey, fobj
